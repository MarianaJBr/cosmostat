...
from cosmostat.chi_square import PARAM_PARTITIONS_GROUP_LABEL, ROOT_GROUP_NAME
from cosmostat.chi_square import BestFitFinder as BaseBestFitFinder
...
class PoolGridExecutor(GridExecutorBase):
    ...
    def map(self, iterator: GridIterator):
        ...
        with Pool(maxtasksperchild=2) as pool_exec:
            pool_imap = pool_exec.imap(
                grid_func, iterator, chunksize=chunk_size
            )
...
class Grid(BaseGrid):
    ...
    def save(
        ...
        grid_matrices = np.meshgrid(*grid_arrays, indexing="ij", sparse=True)
        partition_matrices = dict(zip(partition_names, grid_matrices))
        grid_dataset = group.create_dataset(
            PARTITION_GRID_DATASET_LABEL, shape=grid_shape, dtype="f8"
        )
        ...
        group.create_dataset(
            CHI_SQUARE_DATASET_LABEL, data=self.chi_square_data
        )
