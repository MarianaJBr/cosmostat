...
HEADER_CHAIN = """Chain for 6-D space in the format: (w0, wi, q, zt, h, OmegaM)
for the N steps.

Acceptance ratio: {ratio}
"""
HEADER_LIKE_CHAIN = """Value for the log(Likelihood) calculated
...
def markov_chain(steps, step_width, ini_step, which=BAOSNeHz):
    ...
    w_params = ini_step[0], ini_step[1], ini_step[2], ini_step[3]
    cosmo_params = ini_step[4], ini_step[5]

    like_name, loglike = loglike_set[which]

    chain = [ini_step]
    like_chain = [loglike(w_params, cosmo_params)]
    ...
    for i in range(steps):
        my_rand = np.random.normal(0.0, 1.0, len(ini_step))
        ...
        if not residual:
            print(
                "[Process {}] Markov Chain first {:d} steps "
        ...
        #   like_try = -1e50
            ...
            like_try = loglike(w_params, cosmo_params)

            if np.isnan(like_try):
                print(
                    "[Process {}] Something weird just "
        ...
        rand_uni = np.random.uniform(0.0, 1.0)
    ...
    print(
        "[Process {}] Markov chain acceptance "
...
def export_chains(base_path, chains_specs, chains_results):
    ...
    for index, data in enumerate(chains_data_zip):
        ...
        np.savetxt(
            os.path.join(chain_model_path, chain_file),
            chain,
            header=chain_header,
        )
        np.savetxt(
            os.path.join(chain_model_path, likechain_file),
            like_chain,
            header=like_chain_header,
        )
