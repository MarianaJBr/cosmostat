...
def LikeBAOuncorr(w_params, cosmo_params):
    ...
    return np.exp(-chi2BAOuncorr(w_params, cosmo_params) / 2) / (
        (2 * pi) ** (m / 2) * (1 / np.prod(ERRORS_RBAO_7_NEW)) ** (1 / 2)
    )
...
def LikelihoodSNe(w_params, cosmo_params):
    ...
    m = len(REDSHIFTS_SNE)
    return (
        np.exp(-chi2SNe(w_params, cosmo_params) / 2)
        / ((2 * pi) ** (m / 2) * (1 / np.prod(ERROR_SNE)) ** (1 / 2))
...
def chi2hz(w_params, cosmo_params):
    ...
    for (zi, obs, err) in DATA_HZ:
        chi_sum += (
            (obs - 100 * hubbleflat(zi, w_params, cosmo_params) / H0P) / err
        ) ** 2
...
def likelihoodHz(w_params, cosmo_params):
    ...
    """
    m = len(REDSHIFTS_HZ)
    return np.exp(-chi2hz(w_params, cosmo_params) / 2) / (
        (2 * pi) ** (m / 2) * (1 / np.prod(ERROR_HZ)) ** (1 / 2)
    )
...
def likelihoodCMB(w_params, cosmo_params):
    ...
    return np.exp(-chi2CMB(w_params, cosmo_params) / 2) / (
...
def chi2BAOHz(w_params, cosmo_params):
    ...
    return chi2BAO(w_params, cosmo_params) + chi2hz(w_params, cosmo_params)


def likeBAOHz(w_params, cosmo_params):
    """product of BAO and Hz likelihoods"""

    return likelihoodHz(w_params, cosmo_params) * LikeBAOuncorr(
        w_params, cosmo_params
    )
